...
import { MinigamesReducer } from './minigames.reducers';
...
export interface iGameState {
    ...
    partners: { [partnerId: string]: iPartner } | null //the partners that playing with the player
...
export function gameReducer(state = initialState, action: GameActions.GameActions): iGameState {
    switch (action.type) {
        ...
        case GameActions.UPDATE_NEW_GAMEROOM_DATA:
            const payload: { roomId: string, partnersId: string[] } = action.payload;
        ...
        case GameActions.UPDATE_MINIGAME:
            /*each mini game will handle the 'INITIAL_NEW_MINIGAME' and UPDATE_MINIGAME' differently
                ...
                those functions will be stored in the MinigamesReducer class
                those functions will be pulled from MinigamesReducer to here to be executed
                */
            const miniGameType: GAME_TYPE = action.payload.miniGameType;
            const reducerFunc: (state, action: GameActions.GameActions) => iGameState = MinigamesReducer.getReducerFunction(miniGameType);
